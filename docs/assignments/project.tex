...
October 29, 2020; \textbf{Corresponding author}:
...
\StringTok{  }\NormalTok{pdf\_document}\OperatorTok{:}
\StringTok{    }\NormalTok{citation\_package}\OperatorTok{:}\StringTok{ }\NormalTok{natbib}
\NormalTok{    keep\_tex}\OperatorTok{:}\StringTok{ }\NormalTok{true}
\NormalTok{    fig\_caption}\OperatorTok{:}\StringTok{ }\NormalTok{true}
\NormalTok{    latex\_engine}\OperatorTok{:}\StringTok{ }\NormalTok{pdflatex}
\NormalTok{    template}\OperatorTok{:}\StringTok{ }\ErrorTok{\textasciitilde{}/}\NormalTok{Dropbox}\OperatorTok{/}\NormalTok{miscelanea}\OperatorTok{/}\NormalTok{svm}\OperatorTok{{-}}\NormalTok{r}\OperatorTok{{-}}\NormalTok{markdown}\OperatorTok{{-}}\NormalTok{templates}\OperatorTok{/}\NormalTok{svm}\OperatorTok{{-}}\NormalTok{latex}\OperatorTok{{-}}\NormalTok{ms.tex}
\NormalTok{title}\OperatorTok{:}\StringTok{ "A Pandoc Markdown Article Starter and Template"}
\NormalTok{thanks}\OperatorTok{:}\StringTok{ "Replication files are available on the author\textquotesingle{}s Github account..."}
...
\NormalTok{date}\OperatorTok{:}\StringTok{ "\textasciigrave{}r format(Sys.time(), \textquotesingle{}\%B \%d, \%Y\textquotesingle{})\textasciigrave{}"}
...
\NormalTok{bibliography}\OperatorTok{:}\StringTok{ "\textasciigrave{}r paste0(Sys.getenv(\textquotesingle{}HOME\textquotesingle{}),\textquotesingle{}/Dropbox/master.bib\textquotesingle{})\textasciigrave{}"}
...
\KeywordTok{stargazer}\NormalTok{(}\KeywordTok{lm}\NormalTok{(speed }\OperatorTok{\textasciitilde{}}\StringTok{ }\NormalTok{dist, }\DataTypeTok{data =}\NormalTok{ cars), }\DataTypeTok{title =} \StringTok{"A Handsome Table"}\NormalTok{, }\DataTypeTok{header =} \OtherTok{FALSE}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
